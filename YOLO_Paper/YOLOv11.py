# train_yolov12_coco128_team.py
# -*- coding: utf-8 -*-
"""
YOLOv12 (Ultralytics) 학습/검증/예측 저장 + GT/Pred 좌우 합성 저장
- COCO128 자동 다운로드 및 data.yaml 생성
- mAP@50 / mAP@50-95 출력
- 예측 결과:
  * Ultralytics가 저장한 예측 이미지(박스 그려짐)
  * 우리 커스텀으로 그린 GT/Pred 이미지 + 좌/우 합성본(_paired)
  * YOLO txt + CSV
- 모든 입출력: ./team_project/
"""
...
RUNS_DIR = TP / "_runs_yolov12"
OUT_DIR = TP / "_out_yolov12"
PRED_IMG_DIR = OUT_DIR / "pred_images_v12"  # Ultralytics가 그려준 예측 이미지
...
def main():
    ...
    csv_path = CSV_DIR / "predictions_v12.csv"; df.to_csv(csv_path, index=False, encoding="utf-8")
